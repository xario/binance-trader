...
class Trading():
    ...
    def sell(self, symbol, quantity, orderId, sell_price, last_price):
        ...
        if Orders.get_order(symbol, sell_id)['status'] == 'FILLED':
            ...
            self.option.profit, float(buy_order['price']), sell_price))
    ...
    def action(self, symbol):
        ...
        if self.quantity > 0:
            ...
            if self.max_amount:
                self.amount = float(Orders.get_balance("ETH"))
        ...
        if self.option.prints and self.order_id == 0:
            print('price:%.8f buyp:%.8f sellp:%.8f-bid:%.8f ask:%.8f' % (
        ...
        if self.order_id > 0:
            ...
            profitableSellingPrice = round(
                (profitableSellingPrice - (profitableSellingPrice * self.option.decreasing / 100)), self.tick_size)
